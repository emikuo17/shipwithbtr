"""Streamlit UI for the BTR shipping workflow."""
...
APP_DESCRIPTION = dedent(
    ...
    **English:** Thank you for using BTR. Please complete the form below to finalize your shipment details.
    """
)

RECIPIENT_REQUIREMENTS = dedent(
    """
    **Chinese:** 貨物到台灣後，收件人必須擁有 EZWAY 並完成以下手續：
    1. 實名認證
    2. 委任通關

    **English:** The recipient must complete Real-Name Authentication and authorize Customs Clearance on EZWAY after the package arrives in Taiwan.
    """
)

DROP_OFF_INFO = dedent(
    """
    **Chinese:** 請將包裹交件至以下地址  
    ...
    **English:** If you need assistance delivering your package to this address, please contact us. Additional delivery fees will apply.
    """
)
...
def main() -> None:
    ...
    application_tab, lookup_tab = st.tabs(["Application Form", "Lookup"])

    with application_tab:
        st.subheader("Application Form | 申請表")

        with st.form("application_form", clear_on_submit=True):
            sender_name = st.text_input("Sender's Name (寄件人姓名)", max_chars=100)
            sender_phone = st.text_input("Sender's Phone Number (寄件人電話)", max_chars=25)
            st.markdown(RECIPIENT_REQUIREMENTS)
            recipient_requirements_ack = st.checkbox(
                "Yes, the recipient acknowledges the required steps.",
                value=False,
            )
            recipient_name_zh = st.text_input("Recipient's Name (中文)", max_chars=100)
            recipient_id_number = st.text_input("Recipient's ID Number", max_chars=20)
            recipient_address_zh = st.text_area("Recipient's Address (中文)", max_chars=200)
            recipient_phone = st.text_input("Recipient's Phone Number", max_chars=25)
            preferred_contact_method = st.selectbox(
                "Preferred Contact Method",
                ["Line", "WeChat", "Instagram", "Email", "Phone", "Other"],
            )
            ...
            if submitted:
                ...
                if missing:
                    ...
                    st.error(
                        "Recipient must acknowledge the EZWAY requirements before submission."
                    )
                else:
                    ...
                    st.success(
                        f"Registration complete! Your Customer ID is {customer_id}. Please save this ID for future shipments."
                    )
                    st.json(new_row)
    ...
    with lookup_tab:
        ...
        id_query = st.text_input("Customer ID (e.g., BTR000123)", key="id_search")
        if st.button("Search by ID"):
            if id_query.strip():
                results = df[
                    df["customer_id"].str.fullmatch(id_query.strip(), case=False, na=False)
                ]
                if results.empty:
        ...
        phone_query = st.text_input("Phone number (sender or recipient)", key="phone_search")
        if st.button("Search by phone"):
            if phone_query.strip():
                pattern = phone_query.strip()
                results = df[
                    df["sender_phone"].str.contains(pattern, case=False, na=False)
                    | df["recipient_phone"].str.contains(pattern, case=False, na=False)
                ]
                if results.empty:
                    st.error("No records found.")
                else:
